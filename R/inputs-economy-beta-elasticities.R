...
create_data_inputs_economy_beta_elasticities <- function() {
  ...
  graphs_dir <- paste0(
    working_directory,
    "/graphs/inputs-economy/beta-elasticities"
  )
  create_dir_if_not_exists(graphs_dir)

  results_dir <- paste0(
    working_directory,
    "/results/inputs-economy/beta-elasticities"
  )
  ...
  expenditures_processed <- expenditures %>%
    ...
    filter(
      (fingreen_coicop %in%
        simple_coicop_categories &
        stringi::stri_length(coicop) == 4L) |
        (fingreen_coicop %in%
          split_coicop_categories &
          stringi::stri_length(coicop) == 5L)
    ) %>%
    group_by(geo, time, quantile, fingreen_coicop) %>%
    summarise(
      expenditure_share = sum(values, na.rm = T) / 1000,
      .groups = "drop"
    )
  ...
  potential_references <- expenditures_processed %>%
    ...
    tidyr::pivot_wider(
      id_cols = geo,
      names_from = c("quantile", "fingreen_coicop"),
  ...
  distance_to_fi <- tibble(
    ...
    distance_to_fi = country_distances[, which(
      potential_reference_countries == "FI"
    )]
  )
  ...
  consumption_basket_weights_by_income_group <- expenditures_processed %>%
    group_by(geo, quantile, fingreen_coicop) %>%
    summarise(
      consumption_category_weight = mean(expenditure_share),
      .groups = "drop"
    ) %>%
    group_by(geo, quantile) %>%
    mutate(
      consumption_category_weight = consumption_category_weight /
        sum(consumption_category_weight)
    ) %>%
    ungroup()
  ...
  expenditures_to_filter_out <- expenditures_processed %>%
    ...
    arrange(time) %>%
    mutate(
      d_rel_expenditure_share = abs(
        expenditure_share - lag(expenditure_share)
      ) /
        pmin(expenditure_share, lag(expenditure_share)),
  ...
  expenditures_filtered <- expenditures_processed %>%
    anti_join(
      expenditures_to_filter_out,
      by = c("geo", "time", "fingreen_coicop")
    ) %>%
  ...
  hicp_aggregated <- hicp %>%
    filter(geo %in% analysis_countries) %>%
    inner_join(
      hicp_item_weights_fingreen_coicop,
      by = c("time", "geo", "coicop", "fingreen_coicop")
    ) %>%
  ...
  hicp_selected <- filter(
    hicp_aggregated,
    fingreen_coicop %in% split_coicop_categories
  ) %>%
    bind_rows(
      filter(
        hicp_high_lvl,
  ...
  inflation_faced_by_income_groups <- cumulative_inflation %>%
    ...
    group_by(geo, time, quantile) %>%
    summarise(
      cumulative_inflation_faced_by_group = sum(
        consumption_category_weight * cumulative_inflation
      ),
      .groups = "drop"
    )
  ...
  cumulative_inflation_differences <- cumulative_inflation %>%
    inner_join(
      inflation_faced_by_income_groups,
      by = c("geo", "time"),
      relationship = "many-to-many"
    ) %>%
    mutate(
      difference_in_cumulative_inflation = cumulative_inflation -
        cumulative_inflation_faced_by_group
    ) %>%
    select(
      geo,
      time,
      fingreen_coicop,
      quantile,
      difference_in_cumulative_inflation
    )
  ...
  save_plotly_plot(
    p_cum_inflation_by_coicop_category,
    paste0(graphs_dir, "/cum-inflation-by-coicop-category.html")
  )
  ...
  training <- expenditures_filtered %>%
    inner_join(
      cumulative_inflation_differences,
      by = c("geo", "time", "quantile", "fingreen_coicop")
    )

  # exploration -------------------------------------------------------------

  fit_elasticity_model <- function(
    fingreen_coicop,
    quantile,
    data,
    include_geo = T
    ...
    res <- lm(
      ...
      data = filter(
        data,
        fingreen_coicop == {{ fingreen_coicop }} & quantile == {{ quantile }}
      )
    )
    return(res)
  }
  ...
  # example model
  lm_cp01_i <- fit_elasticity_model("CP01", "QU1", training)

  summary(lm_cp01_i)

  lm_cp01_i_nogeo <- fit_elasticity_model(
    "CP01",
    "QU1",
    training,
    include_geo = F
  ...
  lm_cp03_iii <- fit_elasticity_model("CP03", "QU3", training)

  summary(lm_cp03_iii)

  lm_cp03_iii_nogeo <- fit_elasticity_model(
    "CP03",
    "QU3",
    training,
    include_geo = F
  ...
  lm_cp071_072_v <- fit_elasticity_model("CP071_072", "QU5", training)
  summary(lm_cp071_072_v)

  lm_cp071_072_v_nogeo <- fit_elasticity_model(
    "CP071_072",
    "QU5",
    training,
    include_geo = F
  ...
  elasticities_all %>%
    ...
    labs(
      ...
      subtitle = paste0(
        "Eurostat data. Analysis countries: ",
        paste0(analysis_countries, collapse = ", ")
      )
    )
  ...
  save_plotly_plot(
    p_elasticity_estimates,
    paste0(graphs_dir, "/consumption-elasticity-estimates.html")
  )

  # Undefined p values for CP10 for quantiles 4 and 5, let's check

  lm_cp10_v <- fit_elasticity_model("CP10", "QU5", training)
  summary(lm_cp10_v)
  ...
  save_plotly_plot(
    p_comparison_to_pisa,
    paste0(graphs_dir, "/consumption-elasticity-comparison-to-pisa.html")
  )
  ...
  writexl::write_xlsx(
    res_to_write,
    path = paste0(results_dir, "/beta-elasticities.xlsx")
  )
  return(paste0(results_dir, "/beta-elasticities.xlsx")) # targets package wants the output file path returned
}
