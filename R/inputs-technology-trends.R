create_inputs_technology_trends <- function(raw_data_path, global_params) {
  ...
  catn("Reading the data.")

  io_annual <- readODS::read_ods(
    path = raw_data_path,
    sheet = "io_annual"
  ) |> 
    filter(ind_ava != "U" & ind_use != "U")
  ...
  output_path <- paste0(results_dir, "trends.ods")
