...
### Re-aggregate use table with pefa E_products

```{python}
use_sec_agg_pefa = pd.DataFrame(index=agg_ind.keys(), columns=E_products_PEFA, dtype="float64")
for m in E_products_PEFA:
    for i in agg_ind.keys():
        use_sec_agg_pefa.loc[i, m] = use.loc[m, agg_ind[i]].sum()
...
e_use = pd.read_excel(
    ...
    header=[10],
    index_col=[0],
    sheet_name="emission-relevant-use",
    nrows=39,
    dtype=coltype_dict,
    na_values=[":","m"]
)
t_use = t_use.drop(labels="PROD_NRG (Labels)", axis="index")
t_use = t_use[coltype_dict.keys()]
f_use = f_use.drop(labels="PROD_NRG (Labels)", axis="index")
f_use = f_use[coltype_dict.keys()]
e_use = e_use.drop(labels="PROD_NRG (Labels)", axis="index")
e_use = e_use[coltype_dict.keys()]

e_use.loc[["Electrical energy", "Heat"]] = 0
```

```{python}
t_use_sec_agg = pd.DataFrame(index=agg_ind.keys(), columns=E_products_PEFA, dtype="float64")
for m in E_products_PEFA:
    for i in agg_ind.keys():
        t_use_sec_agg.loc[i, m] = t_use.loc[m, agg_ind[i]].sum()

f_use_sec_agg = pd.DataFrame(index=agg_ind.keys(), columns=E_products_PEFA, dtype="float64")
for m in E_products_PEFA:
    for i in agg_ind.keys():
        f_use_sec_agg.loc[i, m] = f_use.loc[m, agg_ind[i]].sum()

e_use_sec_agg = pd.DataFrame(index=agg_ind.keys(), columns=E_products_PEFA, dtype="float64")
for m in E_products_PEFA:
    for i in agg_ind.keys():
...
```{python}
f_use_share = f_use_sec_agg.div(use_sec_agg_pefa.where(use_sec_agg_pefa != 0, np.nan))
f_use_share.fillna(value=0.0, inplace=True)
```

```{python}
e_use_share = e_use_sec_agg.div(use_sec_agg_pefa.where(use_sec_agg_pefa != 0, np.nan))
e_use_share.fillna(value=0.0, inplace=True)
```

#### Findem

```{python}
use_hh = use.loc[E_products_PEFA, "Total activities by households"].squeeze()
f_use_hh = f_use.loc[E_products_PEFA, "Total activities by households"].squeeze()
e_use_hh = e_use.loc[E_products_PEFA, "Total activities by households"].squeeze()
```

```{python}
f_use_share_hh = f_use_hh.div(use_hh.where(use_hh != 0, np.nan))
f_use_share_hh.fillna(value=0.0, inplace=True)
```

```{python}
e_use_share_hh = e_use_hh.div(use_hh.where(use_hh != 0, np.nan))
e_use_share_hh.fillna(value=0.0, inplace=True)
```
...
# except for final heat use
D_sectors = ["D1", "D2", "D3"]
```

```{python}
f_use_share_D = pd.DataFrame(index=D_sectors, columns=E_products_PEFA, dtype="float64")
for i in D_sectors:
    f_use_share_D.loc[i] = f_use_share.loc["D"]

# Heat, documentation: Physical energy IO, Types of use
for i in ["D2", "D3"]:
...
```{python}
e_use_share_D = pd.DataFrame(index=D_sectors, columns=E_products_PEFA, dtype="float64")
for i in D_sectors:
...
```{python}
# Energy Products aggregation, differing from earlier only by the unaggregated two categories
PEFA_E_prod_oil = [
    "Crude oil, NGL, and other hydrocarbons (excl. bio)",
    "Motor spirit (without bio)",
    "Kerosenes and jet fuels (without bio)",
    "Naphtha",
    "Transport diesel (without bio)", # unaggregated
    "Heating and other gasoil (without bio)", # unaggregated
    "Residual fuel oil",
    "Refinery gas, ethane and LPG",
    "Other petroleum products incl. additives/oxygenates and refinery feedstocks",
]
...
t_use_sec_agg_E_agg = pd.DataFrame(
    index=agg_ind.keys(), columns=PEFA_E_products_agg.keys(), dtype="float64"
)
...
e_use_share_agg_E.fillna(value=0.0, inplace=True)

f_use_share_agg_E = f_use_sec_agg_E_agg / use_sec_agg_E_agg
f_use_share_agg_E.fillna(value=0.0, inplace=True)
```

##### Split D

```{python}
# TODO: this seems to be kind of duplicate work to what we did already without the energy product aggregation.
# See if it could be done only once.
e_use_share_D_agg_E = pd.DataFrame(index=D_sectors, columns=E_products_agg.keys(), dtype="float64")
for i in D_sectors:
...
```{python}
f_use_share_D_agg_E = pd.DataFrame(index=D_sectors, columns=E_products_agg.keys(), dtype="float64")
for i in D_sectors:
...
```{python}
use_hh_E_agg = pd.DataFrame(index=PEFA_E_products_agg.keys(), columns=["hh"], dtype="float64").squeeze()
for m in PEFA_E_products_agg.keys():
    use_hh_E_agg[m] = use_hh[PEFA_E_products_agg[m]].sum()

f_use_hh_E_agg = pd.DataFrame(index=PEFA_E_products_agg.keys(), columns=["hh"], dtype="float64").squeeze()
for m in PEFA_E_products_agg.keys():
    f_use_hh_E_agg[m] = f_use_hh[PEFA_E_products_agg[m]].sum()

e_use_hh_E_agg = pd.DataFrame(index=PEFA_E_products_agg.keys(), columns=["hh"], dtype="float64").squeeze()
for m in PEFA_E_products_agg.keys():
    e_use_hh_E_agg[m] = e_use_hh[PEFA_E_products_agg[m]].sum()
```

```{python}
e_use_share_hh_E_agg = e_use_hh_E_agg.div(use_hh_E_agg.where(use_hh_E_agg != 0, np.nan))
e_use_share_hh_E_agg.fillna(value=0.0, inplace=True)

f_use_share_hh_E_agg = f_use_hh_E_agg.div(use_hh_E_agg.where(use_hh_E_agg != 0, np.nan))
f_use_share_hh_E_agg.fillna(value=0.0, inplace=True)
```
