...
supply = pd.read_excel(
    "source-data/pisa-team-files/PEFA_FI_2011.xlsx",
    header=[10],
    index_col=[0],
    sheet_name="Sheet 1",
    nrows=39,
...
```{python}
supply_sec_agg_E_agg=pd.DataFrame(index=agg_ind.keys(),columns=E_agg,dtype="float64")
for m in E_agg:
...
for i in E_products:
    if i in ["Gas oil and diesel oil"]:
        oth_use.loc[i] = use.loc[E_products_dict[i], oth_use_columns].sum()
    else:
