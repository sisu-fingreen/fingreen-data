...
E_balance_E_prod = pd.DataFrame(
    0, index=E_balance.index, columns=E_products_SIEC.keys()
)
...
```{python}
E_balace_PEFA_frac = E_balance_E_prod.sum().div(
    use_sec_agg.loc["D"].where(use_sec_agg.loc["D"] != 0, np.nan)
```

```{python}
D1_share_E_balance = (
    E_balance_E_prod.sum()
...
e_balance_supply = pd.DataFrame(
    0, index=["D1", "D3"], columns=["Electrical energy", "Heat"]
)
...
# Load x domestic
x_dom = pd.read_excel(
    "source-data/pisa-team-files/WIOD_IT.xlsx",
...
supply_shares = supply_sec_agg.div(
    supply_sec_agg.sum().where(supply_sec_agg.sum() != 0, np.nan)
)
net_supply_shares = net_supply_sec_agg.div(
    net_supply_sec_agg.sum().where(net_supply_sec_agg.sum() != 0, np.nan)
)
...
net_use_shares = net_use_sec_agg.div(
    net_use_sec_agg.sum().where(net_use_sec_agg.sum() != 0, np.nan)
)
...
loco_Z = pd.read_excel(
    "source-data/pisa-team-files/Locomotion_IT.xlsx",
    sheet_name="Z",
    header=[0, 1],
    index_col=[0, 1],
)
...
# HH findem
fd_dom_h_B_E_share = (
    loco_findem.loc["dom", "H"].loc[secB_E].sum() / loco_findem.loc["dom", "H"].loc[secB].sum()
)

# EXP
# remove inventories
B_exp = loco_findem.loc["exp", ["H", "G", "GFCF"]].loc[secB].sum(axis=1) +
    loco_Z.loc[dom, imp_regions].droplevel(level=0, axis=0).loc[secB].sum(axis=1)
...
only_E_buyer["A"] = (
    ...
    .div(Z_dom.loc["A"].where(Z_dom.loc["A"] != 0, np.nan))
    .astype(float)
    .idxmax(skipna=True)
)
```
...
findem_sec_agg = pd.DataFrame(
    index=sectors_D, columns=pd.MultiIndex.from_product([["H", "EX"], E_products])
)
for i in sectors_D:
    for e in E_products:
...
B_Z_imp_s = (
    ...
    .groupby(level=0, axis=0, sort=False)
    .sum()
)
...
impl_price_imp = Z_imp.loc["A", only_E_buyer["A"]] / (
    net_use_sec_agg_imp.loc[
        only_E_buyer["A"], "Wood, wood waste and other solid biomass, charcoal"
    ]
    * net_supply_shares.loc["A", "Wood, wood waste and other solid biomass, charcoal"]
)
```
...
for e in E_products:
    ...
    if (industry_dom_net_E_demand[e] > 0).sum() == 1:
        Z_E_noown[e].loc[
            industry_dom_net_E_supply[e].loc[industry_dom_net_E_supply[e] > 0].index,
            industry_dom_net_E_demand[e].loc[industry_dom_net_E_demand[e] > 0].index,
        ] = industry_dom_net_E_supply[e].loc[industry_dom_net_E_supply[e] > 0].values
    elif (industry_dom_net_E_supply[e] > 0).sum() == 1:
        Z_E_noown[e].loc[
            industry_dom_net_E_supply[e].loc[industry_dom_net_E_supply[e] > 0].index,
            industry_dom_net_E_demand[e].loc[industry_dom_net_E_demand[e] > 0].index,
        ] = industry_dom_net_E_demand[e].loc[industry_dom_net_E_demand[e] > 0].values
...
for e in pot_RAS_E_products:
    if (industry_dom_net_E_supply[e][E_sectors] > 0).sum() == 1:
        Z_E_noown[e].loc[
            industry_dom_net_E_supply.loc[E_sectors, e][industry_dom_net_E_supply.loc[E_sectors, e] > 0].index,
            industry_dom_net_E_demand[e].loc[industry_dom_net_E_demand[e] > 0].index,
        ] = (
            industry_dom_net_E_demand[e].loc[industry_dom_net_E_demand[e] > 0].values
            * net_supply_shares.loc[E_sectors, e].sum()
        )
    else:
...
for e in RAS_E_products:
    relevant_Z = M_Z_E_dom_RAS.loc[
        industry_dom_net_E_supply.loc[E_sectors, e]
        .loc[industry_dom_net_E_supply.loc[E_sectors, e] > 0]
        .index,
        industry_dom_net_E_demand[e].loc[industry_dom_net_E_demand[e] > 0].index,
    ]
    ...
    Z_E_noown[e].loc[
        industry_dom_net_E_supply.loc[E_sectors, e].loc[industry_dom_net_E_supply.loc[E_sectors, e] > 0].index,
        industry_dom_net_E_demand[e].loc[industry_dom_net_E_demand[e] > 0].index,
    ] = RAS(relevant_Z, colsums=colsums, rowsums=rowsums)
...
Z_E_table = pd.DataFrame(
    index=pd.MultiIndex.from_product([E_products, sectors_D]), columns=sectors_D
)
...
for e in E_products_agg.keys():
    Z_E_table_agg_E.loc[e] = (
        ...
        .groupby(level=1, axis=0, sort=False)
        .sum()
        .values
    )
...
```{python}
with pd.ExcelWriter(
    "source-data/pisa-team-files/PEFA_IO_MEZ_RAS_clear_check_jan2025.xlsx",
    engine="openpyxl",
    mode="a",
...
for e in E_products:
    ...
    if (industry_E_imp_demand[e] > 0).sum() == 1:
        Z_E_imp[e].loc[
            industry_E_imp_supply[e].loc[industry_E_imp_supply[e] > 0].index,
            industry_E_imp_demand[e].loc[industry_E_imp_demand[e] > 0].index,
        ] = industry_E_imp_supply[e].loc[industry_E_imp_supply[e] > 0].values
    elif (industry_E_imp_supply[e] > 0).sum() == 1:
        Z_E_imp[e].loc[
            industry_E_imp_supply[e].loc[industry_E_imp_supply[e] > 0].index,
            industry_E_imp_demand[e].loc[industry_E_imp_demand[e] > 0].index,
        ] = industry_E_imp_demand[e].loc[industry_E_imp_demand[e] > 0].values
...
for e in pot_RAS_E_products_imp:
    if (industry_E_imp_supply[e][E_sectors] > 0).sum() == 1:
        Z_E_imp[e].loc[
            industry_E_imp_supply.loc[E_sectors, e][industry_E_imp_supply.loc[E_sectors, e] > 0].index,
            industry_E_imp_demand[e].loc[industry_E_imp_demand[e] > 0].index,
        ] = (
            industry_E_imp_demand[e].loc[industry_E_imp_demand[e] > 0].values
            * imp_net_supply_shares.loc[E_sectors, e].sum()
        )
    else:
...
for e in RAS_E_products_imp:
    ...
    Z_E_imp[e].loc[
        industry_E_imp_supply.loc[E_sectors, e][
            industry_E_imp_supply.loc[E_sectors, e] > 0
        ].index,
        industry_E_imp_demand[e].loc[industry_E_imp_demand[e] > 0].index,
    ] = RAS(relevant_Z, colsums=colsums, rowsums=rowsums)
    # print(e)
...
```{python}
Z_E_imp_table = pd.DataFrame(
    index=pd.MultiIndex.from_product([E_products, sectors_D]), columns=sectors_D
)
A_E_imp_table = pd.DataFrame(
    index=pd.MultiIndex.from_product([E_products, sectors_D]), columns=sectors_D
)
for e in E_products:
...
```{python}
with pd.ExcelWriter(
    "source-data/pisa-team-files/PEFA_IO_MEZ_RAS_jan2025.xlsx",
    engine="openpyxl",
    mode="a",
    if_sheet_exists="replace",
) as writer:
...
for i in sectors_D:
    for h in ["H", "EX"]:
        for e in E_products_agg.keys():
            findem_sec_agg_E_agg.loc[i, (h, e)] = findem_sec_agg.loc[
                i, (h, E_products_agg[e])
            ].sum()
            findem_imp_sec_agg_E_agg.loc[i, (h, e)] = findem_imp_sec_agg.loc[
                i, (h, E_products_agg[e])
            ].sum()
...
source_str_findem = pd.DataFrame(
    index=pd.MultiIndex.from_product([E_products, sectors_D]), columns=["H", "EX"]
)
for i in sectors_D:
    for j in ["H", "EX"]:
        for e in E_products:
...
source_str_findem_agg_E_reorg = pd.DataFrame(
...
for j in ["H", "EX"]:
    for i in sectors_D:
        for e in E_products_agg.keys():
            source_str_findem_agg_E_reorg.loc[(j, i), e] = source_str_findem_agg_E.loc[
                (e, i), j
            ]

source_str_findem_imp_agg_E_reorg = pd.DataFrame(
    index=pd.MultiIndex.from_product([["H", "EX"], sectors_D]),
    columns=E_products_agg.keys(),
)
for j in ["H", "EX"]:
    for i in sectors_D:
        for e in E_products_agg.keys():
            source_str_findem_imp_agg_E_reorg.loc[(j, i), e] = (
                source_str_findem_imp_agg_E.loc[(e, i), j]
            )
```
...
with pd.ExcelWriter(
    ...
    source_str_findem_imp_agg_E.to_excel(
...
```{python}
supply = pd.read_excel(
    "source-data/pisa-team-files/PEFA_IT_2010.xlsx",
    header=[10],
    index_col=[0],
    sheet_name="Sheet 1",
    nrows=39,
...
```{python}
t_use = pd.read_excel(
    "source-data/pisa-team-files/PEFA_IT_2010_use.xlsx",
    header=[10],
    index_col=[0],
    sheet_name="Sheet 2",
    nrows=39,
