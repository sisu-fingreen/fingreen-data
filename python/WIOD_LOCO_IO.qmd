...
os.chdir('objects/')
year=2010
WIOD_cyp=pd.read_excel(f'C:/01_PhD/2nd project/WIOD_data/current_year_prices/WIOT{year}_Nov16_ROW.xlsb', engine='pyxlsb', header=[2,4], index_col=[0,2], sheet_name=f'{year}',skiprows=[5],nrows=2464)
parse_WIOD.set_class_indices(WIOD_cyp)
IO=parse_WIOD(IO=WIOD_cyp)
pickle_out=open(f'WIOD_cyp_{year}.pickle','wb')
...
```{python}
year=2010
os.chdir('C:/01_PhD/3rd project/python_codes/objects')
IO=pickle.load(open(f'WIOD_cyp_{year}.pickle','rb'))
```
...
Z_dom_abr=IO['Z'].loc[parse_WIOD.a_country,parse_WIOD.other_countries].groupby(level=1,axis=1,sort=False).sum()
Z_abr_abr=IO['Z'].loc[parse_WIOD.other_countries,parse_WIOD.other_countries].groupby(level=1,axis=1,sort=False).sum().groupby(level=1,axis=0,sort=False).sum()
Z_imp=IO['Z'].loc[parse_WIOD.other_countries,parse_WIOD.a_country].groupby(level=1,axis=0,sort=False).sum()
Z_dom=IO['Z'].loc[parse_WIOD.a_country,parse_WIOD.a_country]


Z_beta=pd.DataFrame(index=pd.MultiIndex.from_product([beta_index,parse_WIOD.sectors]),columns=pd.MultiIndex.from_product([beta_index,parse_WIOD.sectors]))
...
#
## aggregate sectors
Z_agg_rows=pd.DataFrame(columns=pd.MultiIndex.from_product([beta_index,parse_WIOD.sectors]),index=pd.MultiIndex.from_product([beta_index,parse_WIOD.agg_ind.keys()]))
Z_agg=pd.DataFrame(columns=pd.MultiIndex.from_product([beta_index,parse_WIOD.agg_ind.keys()]),index=pd.MultiIndex.from_product([beta_index,parse_WIOD.agg_ind.keys()]))
for i in beta_index: 
...
x_agg=pd.DataFrame(index=pd.MultiIndex.from_product([parse_WIOD.regions,parse_WIOD.agg_ind.keys()]),columns=['output'])
for i in parse_WIOD.regions:
    for m in parse_WIOD.agg_ind.keys():
        x_agg.loc[(i,m)]=IO['X'].loc[(i,parse_WIOD.agg_ind[m])].sum()
x_beta=pd.DataFrame(index=pd.MultiIndex.from_product([beta_index,parse_WIOD.agg_ind.keys()]),columns=['output'])
x_beta.loc['DOM']=x_agg.loc[parse_WIOD.a_country].values
x_beta.loc['OTH']=x_agg.loc[parse_WIOD.other_countries].groupby(level=1,sort=False,axis=0).sum().values
#
x_beta_inv=1/x_beta
...
```{python}
A_beta=pd.DataFrame(columns=pd.MultiIndex.from_product([beta_index,parse_WIOD.agg_ind.keys()]),index=pd.MultiIndex.from_product([beta_index,parse_WIOD.agg_ind.keys()]))
A_beta=(Z_agg@np.diag(x_beta_inv.squeeze()))
A_beta.columns=A_beta.index
```

```{python}
A_dom_WIOD=A_beta.loc['DOM','DOM']
A_imp_WIOD=A_beta.loc['OTH','DOM']/A_dom_WIOD
...
```{python}
output=pd.read_excel(f'C:/01_PhD/3rd project/Data/Locomotion/economy_R.xlsx', header=[2], index_col=[0], sheet_name='output')
```
...
```{python}
output_vector=pd.DataFrame(index=pd.MultiIndex.from_product([regions,sectors]),columns=['output'])
for r in regions:
    for s in sectors:
...
```{python}
U=pd.read_excel(f'C:/01_PhD/3rd project/Data/Locomotion/economy_R.xlsx', header=[3], index_col=[0,1], sheet_name='amatrix')
U.dropna(axis=0,how='any',inplace=True)

A_impshare=pd.read_excel(f'C:/01_PhD/3rd project/Data/Locomotion/economy_R.xlsx', header=[2], index_col=[0,1], sheet_name='impshare')[sectors]
...
```{python}
A_imp_origin=pd.read_excel(f'C:/01_PhD/3rd project/Data/Locomotion/economy_R.xlsx', header=[1], index_col=[0,1,2], sheet_name='impshareorigin')[sectors]
```
...
agg_region=['DOM','OTH']
Z_region_agg=pd.DataFrame(index=pd.MultiIndex.from_product([agg_region,sectors]),columns=pd.MultiIndex.from_product([agg_region,sectors]))
Z_region_agg.loc['DOM','DOM']=Z.loc[domestic,domestic].values
Z_region_agg.loc['DOM','OTH']=Z.loc[domestic,abroad].groupby(level=1,axis=1,sort=False).sum().values
Z_region_agg.loc['OTH','DOM']=Z.loc[abroad,domestic].groupby(level=1,axis=0,sort=False).sum().values
Z_region_agg.loc['OTH','OTH']=Z.loc[abroad,abroad].groupby(level=1,axis=1,sort=False).sum().groupby(level=1,axis=0,sort=False).sum().values

# Sectors

Z_agg_rows=pd.DataFrame(columns=pd.MultiIndex.from_product([agg_region,sectors]),index=pd.MultiIndex.from_product([agg_region,agg_sector.keys()]))
Z_agg_loco=pd.DataFrame(columns=pd.MultiIndex.from_product([agg_region,agg_sector.keys()]),index=pd.MultiIndex.from_product([agg_region,agg_sector.keys()]))
for i in agg_region: 
...
WIOD_LOCO_Z=pd.DataFrame(columns=pd.MultiIndex.from_product([agg_region,agg_sector.keys()]),index=pd.MultiIndex.from_product([agg_region,agg_sector.keys()]))
for m in agg_region:
    for n in agg_region:
        for i in notD:
            for j in notD:
                WIOD_LOCO_Z.loc[(m,i),(n,j)]=Z_agg.loc[(m,i),(n,j)]
# ROWS
for m in agg_region:
    for n in agg_region:
        for i in DsecLoc:
...
```{python}
with pd.ExcelWriter('C:/01_PhD/3rd project/Data/python_data/WIOD_IT_clear_check.xlsx',engine='openpyxl',mode='a', if_sheet_exists='overlay') as writer:
...
```{python}
findem_loco=pd.read_excel(f'C:/01_PhD/3rd project/Data/Locomotion/economy_R.xlsx', header=[0,1], index_col=[0,1],skiprows=2, sheet_name='finaldemand').loc[:,(slice(None),domestic)]
...
```{python}
findem_loco_exp=pd.read_excel(f'C:/01_PhD/3rd project/Data/Locomotion/economy_R.xlsx', header=[0,1], index_col=[0,1],skiprows=2, sheet_name='finaldemand').loc[domestic,(slice(None),oth_regions)]
...
WIOD_LOCO_Z=pd.read_excel(f'C:/01_PhD/3rd project/Data/python_data/WIOD_IT_clear_check.xlsx', header=[0,1], index_col=[0,1], sheet_name='Z WIOD Loco hybrid')
```
...
```{python}
findem=pd.read_excel('C:/01_PhD/3rd project/Data/python_data/WIOD_IT_clear_check.xlsx',sheet_name='findem wiod-loco',header=[0],index_col=[0,1])
findem_dom=findem.loc['dom']
findem_exp=findem.loc['exp']

Z=pd.read_excel('C:/01_PhD/3rd project/Data/python_data/WIOD_IT_clear_check.xlsx',sheet_name='Z WIOD Loco hybrid',header=[0,1],index_col=[0,1])
Z_dom=Z.loc['DOM','DOM']
Z_exp=Z.loc['DOM','OTH']
Z_imp=Z.loc['OTH','DOM']
```
...
```{python}
with pd.ExcelWriter('C:/01_PhD/3rd project/Data/python_data/WIOD_IT_clear_check_adj_D3.xlsx',engine='openpyxl',mode='a', if_sheet_exists='overlay') as writer:
