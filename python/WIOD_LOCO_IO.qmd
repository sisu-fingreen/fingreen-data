...
parse_WIOD.set_class_indices(WIOD_cyp, country=["FIN"])
IO = parse_WIOD(IO=WIOD_cyp, country=["FIN"])
...
```{python}
with pd.ExcelWriter(
    "source-data/pisa-team-files/WIOD_FI_clear_check.xlsx", engine="openpyxl", mode="w"
) as writer:
...
findem_wiod = {}
findem_wiod["dom"] = IO["Y domestic"].loc["FIN", "FIN"]
findem_wiod["imp"] = IO["Y imports"]["FIN"]
findem_wiod["exp"] = IO["Y exports"].loc["FIN"]
...
WIOD_LOCO_Z = pd.read_excel(
    f"source-data/pisa-team-files/WIOD_FI_clear_check.xlsx",
    header=[0, 1],
    index_col=[0, 1],
...
```{python}
with pd.ExcelWriter(
    "source-data/pisa-team-files/WIOD_FI_clear_check.xlsx",
    engine="openpyxl",
    mode="a",
...
U.dropna(axis=0, how="any", inplace=True)
U.loc["FIN"].sum()["STEAM_HOT_WATER"]

D3_adj_fac = U.loc["FIN"].sum()["STEAM_HOT_WATER"] / A_colsum["D3"]
```
...
```{python}
with pd.ExcelWriter(
    "source-data/pisa-team-files/WIOD_FI_clear_check_adj_D3.xlsx",
    engine="openpyxl",
    mode="w",
) as writer:
