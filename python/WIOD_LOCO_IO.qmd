...
```{python}
# os.chdir('objects/')
WIOD_cyp = pd.read_excel(
    f"source-data/wiod/WIOT{year}_Nov16_ROW.xlsb",
    engine="pyxlsb",
    header=[2, 4],
    index_col=[0, 2],
    sheet_name=f"{year}",
...
parse_WIOD.set_class_indices(WIOD_cyp, country=[country_code3])
IO = parse_WIOD(IO=WIOD_cyp, country=[country_code3])
pickle_out = open(f"objects/WIOD_cyp_{year}.pickle", "wb")
pickle.dump(
    {
        "Z": IO.Z,
        "X": IO.x,
        "Y domestic": IO.y_domestic,
        "Y imports": IO.y_imports,
        "Y exports": IO.y_exports,
        "Z exports": IO.z_exports,
    },
    pickle_out,
    protocol=-1,
)
...
Z_dom_abr = (
    IO["Z"]
    .loc[parse_WIOD.a_country, parse_WIOD.other_countries]
...
Z_abr_abr = (
    ...
    .T.groupby(  # transpose to group by columns, since axis=1 in groupby is deprecated
        level=1, sort=False
    )
    .sum()
    .T.groupby(level=1, sort=False)  # transpose back
    .sum()
)
Z_imp = (
    IO["Z"]
    .loc[parse_WIOD.other_countries, parse_WIOD.a_country]
...
for i in beta_index:
    ...
    for i in beta_index:
        for m in parse_WIOD.agg_ind.keys():
            Z_agg.loc[:, (i, m)] = Z_agg_rows.loc[:, (i, parse_WIOD.agg_ind[m])].sum(
                axis=1
            )
```
...
x_agg = pd.DataFrame(
    index=pd.MultiIndex.from_product([parse_WIOD.regions, parse_WIOD.agg_ind.keys()]),
    columns=["output"],
    dtype="float64",
)
...
x_beta.loc["OTH"] = (
    x_agg.loc[parse_WIOD.other_countries].groupby(level=1, sort=False).sum().values
)
#
x_beta_inv = 1 / x_beta
...
output = pd.read_excel(
    f"source-data/pisa-team-files/economy_R.xlsx",
    header=[2],
    index_col=[0],
    sheet_name="output",
)
...
A_imp_origin = pd.read_excel(
    f"source-data/pisa-team-files/economy_R.xlsx",
    header=[1],
    index_col=[0, 1, 2],
    sheet_name="impshareorigin",
)[sectors]
```
...
A_imp_origin_long = A_imp_origin.melt(
    var_name="to_ind", value_name="share_of_imports", ignore_index=False
).set_index(["to_ind"], append=True)
A_imp_long = A_imp.melt(
    var_name="to_ind", value_name="import_coefficient", ignore_index=False
).set_index(["to_ind"], append=True)
...
A_imp_multireg_long["multiregion_import_coefficient"] = (
    A_imp_multireg_long["share_of_imports"] * A_imp_multireg_long["import_coefficient"]
)
...
Z_region_agg.loc["DOM", "OTH"] = (
    Z.loc[domestic, abroad].T.groupby(level=1, sort=False).sum().T.values
)
Z_region_agg.loc["OTH", "DOM"] = (
    Z.loc[abroad, domestic].groupby(level=1, sort=False).sum().values
)
...
WIOD_LOCO_Z.loc[(agg_region, notD), (agg_region, notD)] = Z_agg.loc[
    (agg_region, notD), (agg_region, notD)
]
    ...
    for n in agg_region:
        for i in DsecLoc:
            for j in notD:
                if (
                    Z_agg.loc[(m, "D"), (n, j)] > 0
                    and (Z_agg_loco.loc[(m, DsecLoc), (n, j)].sum()) == 0
                ):
                    WIOD_LOCO_Z.loc[(m, i), (n, j)] = Z_agg.loc[(m, "D"), (n, j)] / len(
                        DsecLoc
                    )
                else:
                    WIOD_LOCO_Z.loc[(m, i), (n, j)] = Z_agg.loc[(m, "D"), (n, j)] * (
                        Z_agg_loco.loc[(m, i), (n, j)]
                        / (Z_agg_loco.loc[(m, DsecLoc), (n, j)].sum())
                    )
    ...
    for n in agg_region:
        for i in notD:
            for j in DsecLoc:
                if (
                    Z_agg.loc[(m, i), (n, "D")] > 0
                    and (Z_agg_loco.loc[(m, i), (n, DsecLoc)].sum()) == 0
                ):
                    WIOD_LOCO_Z.loc[(m, i), (n, j)] = Z_agg.loc[(m, i), (n, "D")] / len(
                        DsecLoc
                    )
                else:
                    WIOD_LOCO_Z.loc[(m, i), (n, j)] = Z_agg.loc[(m, i), (n, "D")] * (
                        Z_agg_loco.loc[(m, i), (n, j)]
                        / (Z_agg_loco.loc[(m, i), (n, DsecLoc)].sum())
                    )
...
for m in agg_region:
    for n in agg_region:
        for i in DsecLoc:
            for j in DsecLoc:
                WIOD_LOCO_Z.loc[(m, i), (n, j)] = Z_agg.loc[(m, "D"), (n, "D")] * (
                    Z_agg_loco.loc[(m, i), (n, j)]
                    / (Z_agg_loco.loc[(m, DsecLoc), (n, DsecLoc)].sum().sum())
                )
```
...
```{python}
with pd.ExcelWriter(
    f"source-data/pisa-team-files/WIOD_{country_code2}_clear_check.xlsx", engine="openpyxl", mode="w"
) as writer:
...
findem_wiod = {}
findem_wiod["dom"] = IO["Y domestic"].loc[country_code3, country_code3]
findem_wiod["imp"] = IO["Y imports"][country_code3]
findem_wiod["exp"] = IO["Y exports"].loc[country_code3]
findem_wiod["exp"]["Z"] = IO["Z exports"].sum(axis=1).values
```

```{python}
# aggregate sectors:
findem_wiod_agg = pd.DataFrame(
    index=pd.MultiIndex.from_product([findem_cat, parse_WIOD.agg_ind.keys()]),
    columns=findem_wiod["exp"].columns,
    dtype="float64",
)
...
```{python}
findem_loco = pd.read_excel(
    f"source-data/pisa-team-files/economy_R.xlsx",
    header=[0, 1],
    index_col=[0, 1],
    skiprows=2,
    sheet_name="finaldemand",
    ).loc[:, (slice(None), domestic)]
...
```{python}
findem_loco_exp = pd.read_excel(
    f"source-data/pisa-team-files/economy_R.xlsx",
    header=[0, 1],
    index_col=[0, 1],
    skiprows=2,
    sheet_name="finaldemand",
).loc[domestic, (slice(None), oth_regions)]
...
```{python}
findem_loco_dom_imp = pd.DataFrame(
    index=pd.MultiIndex.from_product([["dom", "imp", "exp"], loco_sectors]),
    columns=findem_loco.columns,
    dtype="float64",
)

findem_loco_dom_imp.loc["imp"] = (
    findem_loco.loc[oth_regions].groupby(level=1, sort=False).sum().values
)
findem_loco_dom_imp.loc["dom"] = findem_loco.loc[domestic].values
findem_loco_dom_imp.loc["exp"] = (
    findem_loco_exp.groupby(level=0, sort=False).sum().values
)

findem_loco_cat = pd.DataFrame(
    index=pd.MultiIndex.from_product([["dom", "imp", "exp"], loco_sectors]),
    columns=fd_origin.keys(),
    dtype="float64",
)
...
```{python}
findem_loco_cat_agg_sec = pd.DataFrame(
    index=pd.MultiIndex.from_product([["dom", "imp", "exp"], agg_sector.keys()]),
    columns=fd_origin.keys(),
    dtype="float64",
)
...
```{python}
findem_wiod_cat = pd.DataFrame(
    index=findem_wiod_agg.index, columns=fd_origin.keys(), dtype="float64"
)
...
# Split wiod D-sector according to the ratios in findem_loco_cat_agg_sec, otherwise use aggregated wiod values
for e in ["dom", "imp", "exp"]:
    for i in notD:
        findem_wiod_loco.loc[(e, i)] = findem_wiod_cat.loc[(e, i)]
    for i in DsecLoc:
        for c in fd_origin.keys():
            findem_wiod_loco.loc[(e, i), c] = findem_wiod_cat.loc[(e, "D"), c] * (
                findem_loco_cat_agg_sec.loc[(e, i), c]
                / findem_loco_cat_agg_sec.loc[(e, DsecLoc), c].sum()
            )
```

```{python}
# Add intermediate exports
WIOD_LOCO_Z = pd.read_excel(
    f"source-data/pisa-team-files/WIOD_{country_code2}_clear_check.xlsx",
    header=[0, 1],
    index_col=[0, 1],
...
# This is read as int, convert to float64
Z.loc["DOM", "T"] = Z.loc["DOM", "T"].astype("float64")
...
U.dropna(axis=0, how="any", inplace=True)
U.loc[country_code3].sum()["STEAM_HOT_WATER"]

D3_adj_fac = U.loc[country_code3].sum()["STEAM_HOT_WATER"] / A_colsum["D3"]
```
...
```{python}
# correct X and Z based on the new final demand with inventories excluded
...
```{python}
with pd.ExcelWriter(
    f"source-data/pisa-team-files/WIOD_{country_code2}_clear_check_adj_D3.xlsx",
    engine="openpyxl",
    mode="w",
) as writer:
