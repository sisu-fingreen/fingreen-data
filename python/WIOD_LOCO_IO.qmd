...
```{python}
WIOD_LOCO_Z.fillna(value=0.0, inplace=True)
```
...
```{python}
with pd.ExcelWriter(
    f"source-data/pisa-team-files/WIOD_{country_code2}.xlsx", engine="openpyxl", mode="w"
) as writer:
...
with pd.ExcelWriter(
    ...
    x_new_dom.to_excel(writer, sheet_name="x dom new")
    A_new_dom.to_excel(writer, sheet_name="A new dom")
    A_new_imp.to_excel(writer, sheet_name="A new imp")
    A_new_imp_share.to_excel(writer, sheet_name="A new imp share")
```
